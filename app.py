...
def send_mail():
	...
	try:
		...
		conn_email.close()
		admin_email = (email_row['setting_value'] if email_row and email_row['setting_value'] else app.config.get('MAIL_USERNAME', 'rr3340@naver.com'))
		msg = Message(
			subject=f'[VBE 문의] {name or "익명"} - {email}',
			sender=app.config['MAIL_DEFAULT_SENDER'],
			recipients=[admin_email],
			body=f"새 문의가 접수되었습니다.\n\n이름: {name or '익명'}\n이메일: {email}\n\n내용:\n{message_text}"
		)
		mail.send(msg)
	except Exception as e:
		...
		if user:
			...
			try:
				msg = Message(
					...
					sender=app.config['MAIL_DEFAULT_SENDER'],
					recipients=[email],
					body=f'안녕하세요, Virtual Black Eagles입니다.\n\n임시 비밀번호: {new_pw}\n\n로그인 후 비밀번호를 변경해주세요.'
				)
				mail.send(msg)
				flash('임시 비밀번호가 이메일로 발송되었습니다.', 'success')
			except Exception as e:
				app.logger.error(f"임시 비밀번호 이메일 발송 실패: {str(e)}")
...
def admin_test_email():
	...
	try:
		msg = Message(
			...
			sender=app.config['MAIL_DEFAULT_SENDER'],
			recipients=[test_to],
			body=f'Virtual Black Eagles 이메일 테스트입니다.\n\n이 메일이 도착했다면 SMTP 설정이 정상입니다.\n\n발송 서버: {app.config["MAIL_SERVER"]}:{app.config["MAIL_PORT"]}\n발송 계정: {app.config["MAIL_USERNAME"]}'
		)
		mail.send(msg)
		flash(f'테스트 이메일이 {test_to}로 발송되었습니다! 메일함을 확인하세요.', 'success')
	except Exception as e:
		error_msg = str(e)
		if 'Authentication' in error_msg or '535' in error_msg:
			flash(f'SMTP 인증 실패: 네이버 앱 비밀번호를 확인해주세요. (오류: {error_msg})', 'error')
		elif 'Connection' in error_msg or 'timeout' in error_msg:
			flash(f'SMTP 서버 연결 실패: 서버/포트 설정을 확인해주세요. (오류: {error_msg})', 'error')
		else:
			flash(f'이메일 발송 실패: {error_msg}', 'error')
